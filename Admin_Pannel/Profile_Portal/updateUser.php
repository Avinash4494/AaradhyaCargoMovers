...
$event_id_gen = 'EV'.'-'.rand(100000,999999);
	$employees_id = $_POST['employees_id_info'];
	$dob = $_POST['dob'];
	$gender = $_POST['gender'];
...
if(empty($employees_id)){
		...
		$allowExt  = array('jpeg', 'jpg', 'png', 'gif', 'pdf' , 'docx','doc', 'xls' ,'webp');
			$userPic = $employees_id.'_'.rand(1000,9999).'.'.$imgExt;
		...
		if(!isset($errorMsg)){
			$sql = "insert into admin_info (random_user_id,employees_id_info,dob,gender,alternate_phone,present_address,present_state,present_pincode,permanent_address,permanent_state,permanent_pincode,profile_image)
					values('".$event_id_gen."','".$employees_id."','".$dob."','".$gender."','".$alternate_phone."','".$user_present_address."','".$user_present_state."','".$user_present_pincode."','".$user_permanent_address."','".$user_permanent_state."','".$user_permanent_pincode."','".$userPic."')";
