...
$imgName = $_FILES['medi_image']['name'];
		$imgTmp = $_FILES['medi_image']['tmp_name'];
		$imgSize = $_FILES['medi_image']['size'];
if(empty($total_claimed)){
		...
		if(!isset($errorMsg)){
			$sql = "insert into  raise_mediclaim (claim_id,total_claimed,employees_id,currency,raised_by,bill_date,exp_month,claimCategory,illnessCategory,claim_amount,reason,req_status,raised_on,declareTerms,updated_on,remarks,medi_image)
					values('".$claim_id_gen."','".$total_claimed."','".$employees_id."','".$currency."','".$raised_by."','".$bill_date."','".$exp_month."','".$claimCategory."','".$illnessCategory."','".$claim_amount."','".$reason."','".$req_status."','".$raised_on."','".$declareTerms."','".$updated_on."','".$remarks."','".$userPic."')";
